...
- [ ] 12.6 Builders update affected artifacts (Button and/or Input) per the plan
- [ ] 12.7 Re-run all gates; capture the updated baseline through the approval procedure
- [ ] 12.8 Document the full cycle as a reference example under `design-system/golden-examples/` (or a clearly-named directory under `design-system/feedback/`)

## 13. Memory policy enactment

- [ ] 13.1 Audit existing auto memory entries (if any) for canonical-shaped content; relocate any offenders to canonical files and remove from memory
- [ ] 13.2 Confirm `CLAUDE.md` contains no rule content, token values, dependency information, or procedural workflows
- [ ] 13.3 Confirm no agent prompt or skill references Serena MCP or any external memory orchestrator for canonical state
- [ ] 13.4 Add a short "memory hygiene review" procedure to `.claude/skills/canonical-rule-authoring/SKILL.md` (or its own skill if it grows)

## 14. MVP acceptance verification

- [ ] 14.1 Confirm canonical rules exist for at least Button and Input primitives
- [ ] 14.2 Confirm all four dependency graph JSONs are present and were generated by `generate-graph.mjs` (not hand-edited)
- [ ] 14.3 Confirm Chromatic baselines exist for Button and Input and the Visual Regression Gate passes
- [ ] 14.4 Confirm Button and Input manifests resolve cleanly through the indexes
- [ ] 14.5 Confirm a complete cascading-update plan from §12 is recorded and re-verifiable
- [ ] 14.6 Confirm `CLAUDE.md` and `.claude/rules/` support workflows without holding canonical content
- [ ] 14.7 Tag the corpus version (e.g., `versions/v1.0.0.md`) and announce MVP complete in `changelog.md`
